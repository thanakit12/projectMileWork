                                ...
                                {else}  {* Simple input *}
                                    <input
                                        {if $field.field_type == "P"}
                                                {if $field.field_name == "s_HomeNumber"}
                                                    data-inputmask="'mask': '99-999-9999'"
                                                {else}
                                                    data-inputmask="'mask': '999-999-9999'"
                                                {/if}
                                            {if $field.autocomplete_type}
                                                x-autocompletetype="{$field.autocomplete_type}"
                                            {/if}
                                            type="text"
                                            id="{$id_prefix}elm_{$field.field_id}"
                                            name="{$data_name}[{$data_id}]"
                                            size="32" value="{$value}"
                                            class="phone {if !$skip_field}{$_class}{else}cm-skip-avail-switch{/if}{if $smarty.foreach.profile_fields.index == 0} cm-focus{/if}"
                                            {else}
                                            {if $field.autocomplete_type}
                                                x-autocompletetype="{$field.autocomplete_type}"
                                            {/if}
