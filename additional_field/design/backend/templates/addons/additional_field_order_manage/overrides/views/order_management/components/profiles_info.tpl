...
{function name="profile_enter_data_link"}
    <div class="btn-group"><a class="btn cm-dialog-opener" data-ca-target-id="customer_info"
                              data-ca-scroll="{$scroll_to}">{__("enter_data")}</a></div>
{/function}

{function name="profile_edit_link"}
    {if $is_edit}
        <div class="pull-right">
            <a class="hand cm-tooltip icon-edit cm-dialog-opener{if $click_to} cm-external-click{/if}"
               data-ca-target-id="customer_info" data-ca-scroll="{$scroll_to}"
               {if $click_to}data-ca-external-click-id="{$click_to}"{/if} title="{__("edit")}"></a>
        </div>
    {/if}
{/function}
    ...
    {if !fn_is_empty($user_data)}
        {if $profile_fields.B}
            ...
            {if $user_data.b_phone}
                <a href="tel:{$user_data.b_phone}">
                    <bdi>{$user_data.b_phone}</bdi>
                </a>
            {/if}
        {else}
    ...
    {if !fn_is_empty($user_data)}
        {if $profile_fields.S}
            ...
            {if $user_data.s_phone}
                <a href="tel:{$user_data.s_phone}">
                    <bdi>{$user_data.s_phone}</bdi>
                </a>
            {/if}
...
{capture name="customer_information"}
    {if !fn_is_empty($user_data)}
        ...
        {/if}
        <div class="clear">
            {*Comment user phone out from original*}
            {*            {if $user_data.phone}*}
            {*                <span>{__("phone")}:</span>*}
        ...
        {if $email_changed}
            ...
            <label for="update_customer_details" class="checkbox">
                <input type="checkbox" name="update_customer_details" id="update_customer_details"
                       value="Y"{if $form_id} form={$form_id}{/if}/>
        ...
        {/if}
    {else}
        <input type="hidden" name="user_data_required" id="selected_user_id"
               value="{$selected|default:$customer.user_id}"/>
